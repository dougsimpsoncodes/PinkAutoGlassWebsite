...
export default function GoogleAdsPage() {
  ...
  const leadCounts = useMemo(() => ({
    ...
    texts: googleLeads.filter(l => l.type === 'text').length,
  ...
  const fetchLeads = useCallback(async (period: DateFilter) => {
    try {
      ...
      const res = await fetch(`/api/admin/dashboard/unified-leads?period=${period}&platform=google&market=${market}`, { cache: 'no-store' });
      if (res.ok) {
    ...
    <DashboardLayout>
      ...
      <div className="bg-gradient-to-br from-pink-50 to-purple-50 border-2 border-pink-300 rounded-xl p-6 mb-6 shadow-lg">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
          ...
          {/* Cost Per Lead */}
          <div className="bg-white rounded-lg p-4 shadow-sm border-2 border-pink-300">
            <div className="flex items-center gap-2 mb-1">
              <Target className="w-4 h-4 text-pink-600" />
              <p className="text-sm font-medium text-pink-600">Cost / Lead</p>
            </div>
